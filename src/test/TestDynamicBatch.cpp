...
TestDynamicBatch::TestDynamicBatch(const std::string& path_tex_1, const std::string& path_tex_2) :
  ...
  m_VBO = std::make_unique<VertexBuffer>(nullptr, sizeof(Vertex) * 1000, GL_DYNAMIC_DRAW);
...
std::array<Vertex, 4> TestDynamicBatch::CreateQuad(float x, float y, float textureId) {
  float size = 2.0f;
...
void TestDynamicBatch::OnUpdate(float deltaTime) {
  ...
  auto q1 = CreateQuad(2.0f, 1.0f, 1.0f);
